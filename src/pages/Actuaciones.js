...
import ActuacionesList from "../components/ActuacionesList";

export default function Actuaciones() {
	const [mode, setMode] = useState("grid");
	const { actuaciones, actuacionEnDirecto, getActuaciones, loading } = useActuaciones();
	...
	return (
		<div className="h-screen w-auto p-4 px-2 pt-14 from-slate-800 to-slate-900">
			{loading ? (
				...
				<>
					<Transition show={loading} leave="transition-all ease-in-out duration-300" leaveFrom="opacity-100" leaveTo="opacity-0">
						<div id="default-modal" tabIndex="-1" className="transition-opacity	overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-screen">
							<div className="flex bg-slate-900/90 w-full m-auto mt-auto p-16 h-full">
								<div className="relative bg-white p-16 rounded-lg h-auto max-w-2xl m-auto object-center shadow dark:bg-gray-700">
									<div role="status">
										<svg aria-hidden="true" className="w-16 h-16 text-gray-200 animate-spin fill-slate-700 m-auto" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
											<path
					...
					<div className="container mx-auto pb-3">
						{actuacionEnDirecto[0] ? (
							<div className="mt-4">
								<h1 className="flex items-center lg:text-2xl font-bold text-red-500 animate-pulse mb-4 sm:text-sm md:text-sm">
									¡Estamos tocando!{" "}
									<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={3} stroke="currentColor" className="size-4 animate-pulse text-red-500">
										<path strokeLinecap="round" strokeLinejoin="round" d="M19.5 13.5 12 21m0 0-7.5-7.5M12 21V3" />
									</svg>
								</h1>
								<FeaturedEvent key={1} doc={actuacionEnDirecto[0]} />
							</div>
						) : (
							<div>
								...
								<FeaturedEvent key={2} doc={actuaciones[0]} />
							</div>
						)}
						...
						{mode === "grid" && categories.map((category, index) => <CategoryCarousel key={index} actuaciones={actuaciones} title={category.title} filterValue={category.value} />)}
						{mode === "list" && <ActuacionesList actuaciones={actuaciones} />}
					</div>
				</>
			)}
		</div>
